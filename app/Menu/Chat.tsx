...
export default function Chat() {
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      ...
      {/* CHAT BOX */}
      <div className="flex-1 bg-gray-50 rounded-2xl flex flex-col min-h-0 overflow-hidden border border-gray-100">

        {/* MESSAGES */}
        <div className="flex-1 overflow-y-auto p-3 lg:p-4 space-y-2.5">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`
                max-w-[85%] sm:max-w-[70%] px-3 py-2 rounded-2xl text-xs lg:text-sm leading-relaxed
                ${msg.role === "user"
        ...
        <form
          ...
          className="border-t border-gray-200 p-2.5 lg:p-3 flex gap-2 flex-shrink-0 bg-white"
        >
          <input
            type="text"
            value={input}
            ...
            className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-xs lg:text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 bg-gray-50"
          />
          <button
            type="submit"
            disabled={loading}
            className="px-3 lg:px-4 py-2 rounded-xl bg-primary text-white text-xs lg:text-sm font-semibold disabled:opacity-60 transition flex-shrink-0"
          >
            {loading ? "..." : "Kirim"}
          </button>
        </form>
